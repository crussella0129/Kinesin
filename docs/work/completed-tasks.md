...
## T-108 (direct follow-up)
- **Description:** Replaced the failed demo-first experience with a product-only installed command, per-user introduction/model setup and working-folder choice, human answers/tool activity/slash commands, and explicitly granted native directory creation. Fixed Ctrl+C shutdown and setup delivery, terminal escaping/input bounds, and Unix state/export/backup creation modes while preserving existing bytes and permissions. Windows full suite331 plus final affected CLI15 passed; native Debian full343 and actual PTY first-use/relaunch/folder operations/cancellation passed. Format/all-target Clippy, deny/audit, independent reviews and recorded filesystem/journal verification passed. This is a direct user-requested repair after sprint11; PR11 already merged, so submission uses a follow-up PR. Exact commands, platform hashes, run IDs, failed attempts and limits are recorded in [interactive repair verification](../interactive-entry-repair.md).
- **Intent:** [INT-0029](../intents/INT-0029-interactive-entry-and-workspaces.md)
- **Completed:** 2026-09-12T20:45:42Z
- **Files modified:** Cargo.toml; Cargo.lock; src/cli.rs; src/cli/presentation.rs; src/config.rs; src/lib.rs; src/model.rs; src/onboarding.rs; src/signal.rs; src/storage.rs; src/tools.rs; tests/cli_inspect.rs; tests/replay.rs; tests/runner_tools.rs; tests/windows_signal.rs; README.md; docs/SUMMARY.md; docs/cli.md; docs/configuration.md; docs/getting-started.md; docs/loop-and-tools.md; docs/security.md; docs/supply-chain.md; docs/threat-model.md; docs/verification.md; docs/intents/INT-0029-interactive-entry-and-workspaces.md; docs/interactive-entry-repair.md; docs/work ledgers.
- **Commit:** `3e8484dd0478affa39125721857b5fb73d001d2c`

## T-109 (direct follow-up)
- **Description:** Made local GGUF selection and owned llama-server startup the normal interactive entry, with mandatory working-folder choice, arrow/Enter selection, saved/custom model paths, trusted runtime discovery and explicit external mode. Protected model/runtime parents remain outside tool workspaces; settings changes preserve operator fields and create private backups. Fixed reviewed terminal-lock, Windows canonical-path and signal-registration races. Removed personal environment defaults from active usage examples and documented optional foreground SSH authentication. Windows355 and native Debian368 tests passed, with actual local-model folder/listing and cleanup proof on both; format/Clippy, deny/audit and independent reviews passed. Detailed artifacts, run IDs and limitations are in the linked verification chapter. PR12 is updated without merge.
- **Intent:** [INT-0007](../intents/INT-0007-managed-model-process.md), [INT-0029](../intents/INT-0029-interactive-entry-and-workspaces.md)
...
- **Commit:** `f886d3d8329015a520bfdaf87b69a53b5b4f9830`
