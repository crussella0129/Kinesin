...
**Every implementation step below is unbuilt.** This review created a plan and
guide, not a working Rust harness. The completed documentation review is tracked
separately in [research](research.md).
...
- [ ] 1. [Write the two operating profiles](build-guide.md#1-write-the-two-operating-profiles).
- [ ] 2. [Set up Rust and an ownership scratch project](build-guide.md#2-set-up-rust-and-an-ownership-scratch-project).
- [ ] 3. [Create one package and a repeatable checkpoint](build-guide.md#3-create-one-package-and-a-repeatable-checkpoint).
- [ ] 4. [Prove the model contract independently](build-guide.md#4-prove-the-model-contract-independently).
- [ ] 5. [Define owned domain types](build-guide.md#5-define-owned-domain-types).
- [ ] 6. [Make state transitions without I/O](build-guide.md#6-make-state-transitions-without-io).
- [ ] 7. [Learn enough async to run the shell](build-guide.md#7-learn-enough-async-to-run-the-shell).
- [ ] 8. [Run a scripted model through one runner](build-guide.md#8-run-a-scripted-model-through-one-runner).
- [ ] 9. [Validate configuration and construct run authority](build-guide.md#9-validate-configuration-and-construct-run-authority).
- [ ] 10. [Persist a run through one SQLite owner](build-guide.md#10-persist-a-run-through-one-sqlite-owner).
- [ ] 11. [Map the wire format with saved fixtures](build-guide.md#11-map-the-wire-format-with-saved-fixtures).
- [ ] 12. [Add the bounded HTTP adapter](build-guide.md#12-add-the-bounded-http-adapter).
- [ ] 13. [Complete the first live command](build-guide.md#13-complete-the-first-live-command).
- [ ] 14. [Enforce stopping and cancellation around effects](build-guide.md#14-enforce-stopping-and-cancellation-around-effects).
...
- [ ] 15. [Define the two tool contracts](build-guide.md#15-define-the-two-tool-contracts).
- [ ] 16. [Create the workspace capability](build-guide.md#16-create-the-workspace-capability).
- [ ] 17. [Implement a bounded file read](build-guide.md#17-implement-a-bounded-file-read).
- [ ] 18. [Implement a bounded directory listing](build-guide.md#18-implement-a-bounded-directory-listing).
- [ ] 19. [Complete one model/tool/model round trip](build-guide.md#19-complete-one-modeltoolmodel-round-trip).
- [ ] 20. [Prove policy, protocol, and loop failure paths](build-guide.md#20-prove-policy-protocol-and-loop-failure-paths).
- [ ] 21. [Check file fields before accepting a task](build-guide.md#21-check-file-fields-before-accepting-a-task).
- [ ] 22. [Evaluate the useful read-only agent](build-guide.md#22-evaluate-the-useful-read-only-agent).
...
- [ ] 23. [Make each concurrent run own its state](build-guide.md#23-make-each-concurrent-run-own-its-state).
- [ ] 24. [Bound admission, queues, bytes, and waiters](build-guide.md#24-bound-admission-queues-bytes-and-waiters).
- [ ] 25. [Separate active runs from model capacity](build-guide.md#25-separate-active-runs-from-model-capacity).
- [ ] 26. [Bound blocking tool work by its real lifetime](build-guide.md#26-bound-blocking-tool-work-by-its-real-lifetime).
- [ ] 27. [Shut down and cancel without losing ownership](build-guide.md#27-shut-down-and-cancel-without-losing-ownership).
- [ ] 28. [Add a batch command for independent agents](build-guide.md#28-add-a-batch-command-for-independent-agents).
- [ ] 29. [Measure saturation before tuning concurrency](build-guide.md#29-measure-saturation-before-tuning-concurrency).
- [ ] 30. [Inspect stored runs and replay decisions](build-guide.md#30-inspect-stored-runs-and-replay-decisions).
- [ ] 31. [Stream output with bounded assembly](build-guide.md#31-stream-output-with-bounded-assembly).
...
- [ ] 34. [Define and implement private API ingress](build-guide.md#34-define-and-implement-private-api-ingress).
- [ ] 35. [Authenticate owners with provisioned credentials](build-guide.md#35-authenticate-owners-with-provisioned-credentials).
- [ ] 36. [Persist owner-scoped results and idempotent creation](build-guide.md#36-persist-owner-scoped-results-and-idempotent-creation).
- [ ] 37. [Give each owner bounded and fair capacity](build-guide.md#37-give-each-owner-bounded-and-fair-capacity).
- [ ] 38. [Harden the shared filesystem and service process](build-guide.md#38-harden-the-shared-filesystem-and-service-process).
- [ ] 39. [Recover interruption without inventing execution history](build-guide.md#39-recover-interruption-without-inventing-execution-history).
- [ ] 40. [Operate retention, backups, readiness, and overload](build-guide.md#40-operate-retention-backups-readiness-and-overload).
